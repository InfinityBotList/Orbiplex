...
export function CollectionCarousel({
    ...
    // Safe children array with fallback
    const safeChildren = children || []
    const totalSlides = Math.ceil(safeChildren.length / viewItems)
    ...
    const slides = []
    for (let i = 0; i < safeChildren.length; i += viewItems) {
        slides.push(safeChildren.slice(i, i + viewItems))
    }

    // Return loading state if no children
    if (safeChildren.length === 0) {
        return (
            <div className={cn('relative', className)}>
                <div className="overflow-hidden rounded-2xl bg-card/80 backdrop-blur-md shadow-lg border border-border/50 p-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {Array.from({ length: itemsPerView }).map((_, i) => (
                            <div key={`loading-${i}`} className="h-64 bg-muted rounded-lg animate-pulse" />
                        ))}
                    </div>
                </div>
            </div>
            ...
            >
                <AnimatePresence mode="wait">
                    <motion.div
                        ...
                        className={cn('grid transition-all', `grid-cols-${viewItems} gap-${gap / 4}`)}
                        style={{
                    ...
                    {Array.from({ length: totalSlides }).map((_, idx) => (
                        <button
                            ...
                            className={cn(
                                ...
                                currentIndex === idx ? 'bg-primary scale-110' : 'bg-primary/30 hover:bg-primary/50'
                            )}
